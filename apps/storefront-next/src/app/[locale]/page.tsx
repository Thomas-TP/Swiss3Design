...
import { FREE_SHIPPING_OVER_AMOUNT } from "@/lib/shipping";
...
async function fetchFeaturedProducts(): Promise<StoreProductSummary[]> {
  ...
  return products
    ...
    .sort(
      (a, b) =>
        ((a.metadata?.featured_order as number) ?? 0) - ((b.metadata?.featured_order as number) ?? 0),
    )
    .map((p) => ({
      id: p.id,
      title: p.title,
  ...
  const trust = [
    {
      ...
      text: t("trustShippingText", { amount: formatChfAmount(FREE_SHIPPING_OVER_AMOUNT, locale) }),
    },
    { Icon: Factory, title: t("trustMadeTitle"), text: t("trustMadeText") },
    { Icon: ShieldCheck, title: t("trustPaymentTitle"), text: t("trustPaymentText") },
  ];
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6">
      ...
      <section className="grid items-center gap-10 py-14 md:grid-cols-2 md:gap-14 md:py-24">
        <div>
          ...
          <Reveal delay={0.16}>
            <p className="mt-5 max-w-md text-lg leading-relaxed text-soft">{t("heroSubtitle")}</p>
          </Reveal>
          <Reveal delay={0.24}>
      ...
      <section className="grid gap-3 border-t border-line py-10 sm:grid-cols-3 sm:gap-4">
        {trust.map(({ Icon, title, text }) => (
          <div key={title} className="flex items-start gap-3.5 rounded-2xl border border-line bg-surface p-4 transition-colors hover:border-soft/40">
      ...
      <Reveal inView>
        <section className="relative grid gap-8 overflow-hidden rounded-card bg-night p-8 text-white ring-1 ring-night-line md:grid-cols-[1.2fr_1fr] md:items-center md:p-14">
          <div aria-hidden className="pointer-events-none absolute -left-16 -top-16 h-64 w-64 rounded-full bg-accent/20 blur-3xl" />
          <div className="relative">
            <MulticolorDots size={12} onDark />
            <h2 className="mt-5 text-3xl font-bold tracking-tight md:text-4xl">{t("multicolorTitle")}</h2>
            <p className="mt-4 max-w-lg leading-relaxed text-night-soft">{t("multicolorText")}</p>
          </div>
          <div aria-hidden className="relative hidden h-44 items-end gap-3 md:flex">
            {["#e5231c", "#1d4ed8", "#f59e0b", "#fafaf9"].map((c, i) => (
              <span key={c} className="flex-1 rounded-t-xl" style={{ backgroundColor: c, height: `${55 + i * 15}%` }} />
            ))}
          </div>
        </section>
      </Reveal>
      ...
      <Reveal inView>
        <section className="py-16">
          ...
          <h2 className="mt-3 text-2xl font-bold tracking-tight md:text-3xl">{t("processTitle")}</h2>
          <ol className="mt-8 grid gap-4 sm:grid-cols-3">
            {([1, 2, 3] as const).map((n) => (
              <li key={n} className="relative rounded-card border border-line bg-surface p-6">
                ...
                <p className="mt-1.5 text-sm leading-relaxed text-soft">
                  {t(`processStep${n}Text`, { amount: formatChfAmount(FREE_SHIPPING_OVER_AMOUNT, locale) })}
                </p>
              </li>
            ))}
          </ol>
      ...
      <section className="pb-16">
        <div className="mb-7 flex items-end justify-between gap-4">
          <div>
            <span className="flex h-1 w-10 rounded-full bg-accent" />
            <h2 className="mt-3 text-2xl font-bold tracking-tight md:text-3xl">{t("featuredTitle")}</h2>
          </div>
          <Link
            href="/shop"
      ...
      <Reveal inView>
        <section className="mb-16 rounded-card border border-line bg-surface p-8 text-center md:p-12">
          ...
          <h2 className="mt-4 text-2xl font-bold tracking-tight md:text-3xl">{t("customBandTitle")}</h2>
          <p className="mx-auto mt-3 max-w-xl leading-relaxed text-soft">{t("customBandText")}</p>
          <Link
            href="/custom"
