...
export function RegisterForm({ defaultEmail = "" }: { defaultEmail?: string }) {
  ...
  useEffect(() => {
    ...
    const attempt = async () => {
      ...
      const { data, error: err } = await signIn.email(
        { email: waiting.email, password: waiting.password },
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (err) {
      setError(err.status === 422 ? t("errorExists") : err.status === 400 ? t("errorWeakPassword") : t("errorGeneric"));
      setPending(false);
      return;
    }
  ...
  if (waiting) {
    return (
      <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center">
        ...
        <p className="mt-3 text-sm font-medium leading-relaxed text-emerald-800 dark:text-emerald-200">{t("verifyNotice")}</p>
