...
export function PasskeysSection() {
  ...
  useEffect(() => {
    ...
    authClient.passkey.listUserPasskeys().then(({ data }) => {
      if (!cancelled) setPasskeys((data as PasskeyRow[] | null) ?? []);
    });
    return () => {
      cancelled = true;
    };
  ...
  async function onAdd() {
    setPending("add");
    setError(null);
    const { browser, os } = describeUserAgent(typeof navigator !== "undefined" ? navigator.userAgent : null);
    const name = browser && os ? `${browser} sur ${os}` : new Date().toLocaleDateString();
    const { error: err } = await authClient.passkey.addPasskey({ name });
    setPending(null);
    if (err) {
  ...
  return (
    <div className={card}>
          ...
          {passkeys.map((p) => (
            <li key={p.id} className="flex items-center justify-between gap-3 py-3">
              <div className="min-w-0">
                <p className="truncate text-sm font-medium">{p.name || t("security.passkeys.unnamed")}</p>
                {p.createdAt && (
                  <p className="mt-0.5 text-xs text-soft">{new Date(p.createdAt).toLocaleDateString(`${locale}-CH`)}</p>
                )}
              </div>
              <button
                type="button"
      ...
      <button type="button" onClick={onAdd} disabled={pending !== null} className={`${btnPrimary} mt-4`}>
        <Plus size={15} />
