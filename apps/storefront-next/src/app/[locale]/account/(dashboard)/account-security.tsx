...
function TwoFactor() {
  ...
  // erroné répond 400 : tout le reste (500, réseau, session expirée...) n'a
  // rien à voir avec le mot de passe et ne doit jamais afficher "mot de passe
  // incorrect" (cf. incident racine du 2026-07-02, même logique ici).
  ...
  async function onRegenerateBackupCodes() {
    setPending(true);
    setError(null);
    const { data: res, error: err } = await twoFactor.generateBackupCodes({ password });
    setPending(false);
    if (err || !res) {
      setError(errorMessage(err, t("security.errorPassword")));
      return;
    ...
    <div className="rounded-card border border-line bg-surface p-5 sm:p-6">
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold ${
            enabled ? "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300" : "bg-line text-soft"
          }`}
        >
      ...
      {setup ? (
        <div className="mt-5 space-y-4">
          ...
          <div className="flex flex-col items-center gap-3 sm:flex-row sm:items-start">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold text-soft">{t("security.backupTitle")}</p>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              />
              <button type="button" onClick={onConfirm} disabled={code.length !== 6 || pending} className={btnPrimary}>
          ...
          {newBackupCodes ? (
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2.5">
                <button type="button" onClick={onDisable} disabled={pending || !password} className={btnGhost}>
                  <ShieldOff size={15} />
                  {pending ? t("security.processing") : t("security.disable")}
                </button>
                <button type="button" onClick={onRegenerateBackupCodes} disabled={pending || !password} className={btnGhost}>
                  <KeyRound size={15} />
        ...
        <div className="mt-4 space-y-3">
          ...
          {error && <p className="text-sm font-medium text-accent">{error}</p>}
          <button type="button" onClick={onEnable} disabled={pending} className={btnPrimary}>
            <KeyRound size={15} />
