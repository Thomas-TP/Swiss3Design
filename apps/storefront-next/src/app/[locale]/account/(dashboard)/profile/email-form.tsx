...
export function EmailForm({ email, verified }: { email: string; verified: boolean }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setPending(true);
    setError(null);
    const { error: err } = await changeEmail({ newEmail: value.trim().toLowerCase() });
    setPending(false);
    if (err) {
  ...
  return (
    <div className="mt-3">
      ...
      {!verified && !editing && (
        <button type="button" onClick={resendVerification} disabled={pending || resendSent} className={`${btnGhost} mt-3`}>
      ...
      {editing && (
        <form onSubmit={onSubmit} className="mt-3 flex flex-col gap-2.5 sm:flex-row">
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={pending || !value.trim()} className={btnPrimary}>
