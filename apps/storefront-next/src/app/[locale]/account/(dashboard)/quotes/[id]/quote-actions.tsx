...
import { ArrowRight, Pencil, X, Paperclip, Send, Ban, Check } from "lucide-react";
...
function RevisePanel({ quoteId, onDone, onClose }: { quoteId: string; onDone: () => void; onClose: () => void }) {
  const t = useTranslations("account.quoteActions");
  ...
  async function onFile(input: HTMLInputElement) {
    ...
    try {
      ...
      const res = await fetch(`${MEDUSA_URL}/store/quotes/upload`, {
        method: "POST",
        headers: { "x-publishable-api-key": PUBLISHABLE_KEY, ...(token ? { Authorization: `Bearer ${token}` } : {}) },
        body,
      });
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setError(false);
    try {
      await medusa.client.fetch(`/store/quotes/${quoteId}/messages`, {
        method: "POST",
        body: { body: message, file_url: file?.url, file_name: file?.name },
      });
      onDone();
    } catch {
      setError(true);
  ...
  return (
    <form onSubmit={onSubmit} className="mt-3 space-y-4 rounded-card border border-line bg-surface p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold">{t("reviseTitle")}</p>
          <p className="mt-0.5 text-xs text-soft">{t("reviseDesc")}</p>
        </div>
        <button type="button" onClick={onClose} aria-label={t("cancel")} className="rounded-full p-1 text-soft transition-colors hover:bg-line/60 hover:text-ink">
          <X size={16} />
        </button>
      </div>
      ...
      {file ? (
        <div className="flex items-center justify-between gap-3 rounded-xl border border-line bg-paper px-4 py-3 text-sm">
          ...
          <button type="button" onClick={() => setFile(null)} aria-label="×" className="rounded-full p-1 text-soft transition-colors hover:bg-line/60 hover:text-accent">
            <X size={15} />
          </button>
        </div>
      ) : (
        <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border-2 border-dashed border-line px-4 py-3.5 text-sm font-medium text-soft transition-colors hover:border-ink hover:text-ink">
          ...
          <input type="file" accept=".stl,.3mf,.obj" disabled={uploading} onChange={(e) => onFile(e.currentTarget)} className="hidden" />
        </label>
      )}
      {fileError && <p className="text-xs font-medium text-accent">{t("fileError")}</p>}

      {error && <p className="rounded-xl bg-accent/10 px-4 py-3 text-sm font-medium text-accent">{t("error")}</p>}
...
function DeclinePanel({ quoteId, onDone, onClose }: { quoteId: string; onDone: () => void; onClose: () => void }) {
  const t = useTranslations("account.quoteActions");
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setError(false);
    try {
      await medusa.client.fetch(`/store/quotes/${quoteId}/decline`, {
        method: "POST",
  ...
  return (
    <form onSubmit={onSubmit} className="mt-3 space-y-4 rounded-card border border-line bg-surface p-5">
      ...
      <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} placeholder={t("declineReasonPlaceholder")} className={field} />
