...
import { ArrowRight, Check, ChevronDown, Lock, Mail, MapPin, Printer, ShieldCheck, Truck } from "lucide-react";
...
function SectionHeading({ kicker, title }: { kicker: string; title: string }) {
  return (
    <div className="mb-8">
      ...
      <p className="mt-3 text-sm font-semibold uppercase tracking-wide text-accent">{kicker}</p>
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6">
      ...
      <section className="py-14 md:py-20">
        ...
        <Reveal delay={0.08}>
          <h1 className="mt-6 max-w-3xl text-[2.3rem] font-bold leading-[1.08] tracking-tight md:text-5xl">{c.title}</h1>
        </Reveal>
        <Reveal delay={0.16}>
          <p className="mt-5 max-w-2xl text-lg leading-relaxed text-soft">{c.intro}</p>
        </Reveal>
      </section>
        ...
        <section className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
          {c.stats.map((s) => (
            <div key={s.label} className="rounded-2xl border border-line bg-surface p-5 text-center">
              <p className="text-2xl font-bold tracking-tight text-ink md:text-3xl">{s.value}</p>
        ...
        <div className="grid gap-8 md:grid-cols-2 md:gap-10 md:items-start">
          <Reveal inView>
            <figure className="overflow-hidden rounded-card border border-line bg-white shadow-sm shadow-ink/[0.06] dark:shadow-black/30">
              ...
              <img src="/about/p1s-ams2-pro.jpg" alt={c.equipmentTitle} width={1340} height={1420} loading="lazy" className="block h-auto w-full" />
            </figure>
          </Reveal>
          <Reveal inView delay={0.1}>
            <p className="text-[15px] leading-relaxed text-soft">{c.equipmentText}</p>
            <div className="mt-6 rounded-card border border-line bg-elevated p-6 shadow-sm shadow-ink/[0.04]">
              ...
              <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-soft">{c.specsTitle}</p>
          ...
          {c.steps.map((step, i) => (
            <Reveal key={step.title} inView delay={i * 0.06}>
              <div className="flex h-full flex-col rounded-2xl border border-line bg-surface p-5">
                <span className="grid h-9 w-9 place-items-center rounded-full bg-accent/10 text-sm font-bold text-accent">{i + 1}</span>
                <p className="mt-4 text-sm font-semibold">{step.title}</p>
                <p className="mt-1.5 text-sm leading-snug text-soft">{step.text}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <section className="border-t border-line py-16 md:py-20">
        ...
        <Reveal inView>
          <p className="mb-7 max-w-2xl text-[15px] leading-relaxed text-soft">{c.materialsText}</p>
        </Reveal>
        <Reveal inView delay={0.08}>
          <div className="grid gap-8 rounded-card border border-line bg-elevated p-7 md:grid-cols-[0.8fr_1.2fr] md:items-center md:p-9">
            <div>
              <p className="text-5xl font-bold tracking-tight md:text-6xl">{c.plaName}</p>
              <p className="mt-2 text-sm font-medium text-accent">{c.plaTagline}</p>
            </div>
        ...
        <Reveal inView>
          <p className="mt-5 text-sm leading-relaxed text-soft">{c.materialsNote}</p>
        </Reveal>
      </section>
            ...
            return (
              <Reveal key={item.title} inView delay={i * 0.06}>
                <div className="flex h-full flex-col rounded-2xl border border-line bg-surface p-5 transition-colors hover:border-soft/40">
                  ...
                  <p className="mt-1 text-sm leading-snug text-soft">{item.text}</p>
                </div>
              </Reveal>
            );
          })}
      ...
      <section className="border-t border-line py-16 md:py-20">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="space-y-3">
            {c.faq.map((item) => (
              <details key={item.q} className="faq-item rounded-2xl border border-line bg-surface px-5 transition-colors hover:border-soft/40">
                <summary className="flex cursor-pointer items-center justify-between gap-4 py-4 text-[15px] font-semibold">
                  {item.q}
                  <ChevronDown size={18} className="faq-chevron shrink-0 text-soft transition-transform duration-200" />
                </summary>
                <div className="pb-5 text-[15px] leading-relaxed text-soft">{item.a}</div>
              </details>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-2xl">
          <Reveal inView>
            <div className="text-center">
              ...
              <p className="mt-3 text-sm font-semibold uppercase tracking-wide text-accent">{c.contactKicker}</p>
              <h2 className="mt-1 text-2xl font-bold tracking-tight md:text-3xl">{c.contactTitle}</h2>
              <p className="mx-auto mt-3 max-w-md leading-relaxed text-soft">{c.contactText}</p>
            </div>
          </Reveal>
          ...
          <Reveal inView>
            <div className="mt-6 flex flex-col items-center justify-center gap-3 text-sm text-soft sm:flex-row sm:gap-6">
              ...
              <Link href="/custom" className="inline-flex items-center gap-1.5 font-medium text-ink transition-colors hover:text-accent">
                {t("quoteCta")}
                <ArrowRight size={15} />
              </Link>
            </div>
