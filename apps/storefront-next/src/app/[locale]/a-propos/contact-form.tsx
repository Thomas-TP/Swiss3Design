...
// Envoi via une route cross-origine sur l'app racine (src/app/api/contact-message,
// mêmes gabarits d'e-mail/logique anti-spam que le Server Action original) :
// storefront-next n'a pas d'accès direct à Resend/aux e-mails admin.
export function ContactForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`${BETTER_AUTH_URL}/api/contact-message`, {
        method: "POST",
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div>
        <label htmlFor="subject" className="mb-1.5 block text-sm font-semibold">
          {t("subject")} <span className="font-normal text-soft">({t("optional")})</span>
        </label>
      ...
      <div>
        ...
        <textarea id="message" name="message" required minLength={10} rows={5} placeholder={t("messagePlaceholder")} className={field} />
      </div>
